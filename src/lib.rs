...
fn process_stylesheet(
    ...
    let mut stylesheet = match StyleSheet::parse(
        code,
        mk_parser_options(filename, error_recovery, parser_flags),
    ) {
        Ok(val) => val,
        Err(e) => {
            return Err(PyValueError::new_err(format!(
                "Parsing stylesheet failed: {}",
                e.to_string()
            )))
        }
    };
...
fn calc_parser_flags(nesting: bool, custom_media: bool, deep_selector_combinator: bool) -> u8 {
    ...
    if deep_selector_combinator {
        flags |= 1 << 2;
    }
    return flags;
}

fn mk_parser_options(filename: &str, error_recovery: bool, parser_flags: u8) -> ParserOptions {
...
fn mk_minify_options(unused_symbols: Option<HashSet<String>>, targets: &Targets) -> MinifyOptions {
...
fn mk_printer_options<'a>(targets: &Targets, minify: bool) -> PrinterOptions<'a> {
